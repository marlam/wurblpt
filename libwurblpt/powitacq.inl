...
public:
    ...
    float eval(Vector2f pos, const float *param = nullptr) const {
        ...
        for (ssize_t dim = 0; dim < ssize_t(Dimension); ++dim) {
            if (m_param_size[dim] == 1) {
                param_weight[2 * dim] = 1.f;
                param_weight[2 * dim + 1] = 0.f;
                continue;
...
float BRDF::eval(size_t wavelengthIndex, const Vector3f &wi, const Vector3f &wo) const {
    if (wi.z() <= 0 || wo.z() <= 0)
        return 0.0f;
    ...
    float params_fr[3] = { phi_i, theta_i, m_data->wavelengths[wavelengthIndex] };
    float fr = m_data->spectra.eval(sample, params_fr);
...
float BRDF::sample(size_t wavelengthIndex,
                      const Vector2f &u, const Vector3f &wi,
                      Vector3f *wo_out, float *pdf_out) const {
    if (wi.z() <= 0) {
        if (wo_out)
            *wo_out = Vector3f(0.f);
